...
function SelectedTerm(props){
  var terms = ['Germany'];

  return(
    <ul className='terms'>
    {terms.map(function (ter){
      return(
        <li 
        style={ter === props.selectedTerm ? { color: '#d0021b'} : null
      }
      onClick={props.onSelect.bind(null, ter)}
      key={lang}>
      {ter}
      </li>
      )
...
//AJAX call
componentDidMount(){
API.runQuery(this.state.selectedTerm)
.then(function(repos){
  console.log(repos)
})
}
